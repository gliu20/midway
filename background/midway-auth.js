...
midway.auth.getIdToken = async function () {
    ...
    const tokenRequestUrl = `https://securetoken.googleapis.com/v1/token?key=${midway.config.firebase.apiKey}`;
...
midway.auth.getUserDetails = async function () {
    ...
    const requestUrl = `https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${midway.config.firebase.apiKey}`;
